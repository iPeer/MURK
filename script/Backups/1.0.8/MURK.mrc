...
alias -l player.legsbasehp {
  var %basehp $iif($readini(MURK\data\Weapon.ini,$replace($player.legsbase,$chr(32),-),HP),$v1,0)
  $iif($isid,return,murkc) %basehp
}
...
alias -l player.unequipitem {
  ...
  elseif ($readini(MURK\data\Item.ini,$1,IsLegs)) {
    var %name $readini(MURK\data\Item.ini,$1,Name)
...
alias -l player.checkquant {
  ...
  if ($1 == 1) {
    if ($hget(MURK.Player,Coins) == $null) {
      hadd MURK.Player Coins $player.checkquant(1,return)
...
on *:dialog:MURK.Armoury:*:*:{
  if ($devent == init) {
    ...
    if ($player.legshp < $player.legsbasehp && $player.checkquant(1,$readini(MURK\data\Legs.ini,$replace($player.shieldbase,$chr(32),-),COST))) { did -ve $dname 19 }
...
on *:dialog:MURK.Fight:sclick:*:{
  ...
  if ($did == 15) { 
    ...
    else { .timerMurk.Combat.Monster.Attack off | player.removeitem 1 $ceil($player.mhp) | hdel MURK.Player Monster.HP | dialog -x $dname | if ($dialog(MURK.Hub)) { did -e MURK.Hub 35,43 } }
  }
  if ($did == 14) { 
    if (!$dialog(MURK.Hub)) { dialog -md MURK.Hub MURK.Hub }
    else dialog -ve MURK.Hub
...
alias -l murk.combat.player {
  ...
  if ($player.weaponhp == 0 && $player.weaponbasehp) { var %playermaxd $calc(%playermaxd * .10), %playermind $calc(%playermind * .10) }
...
alias -l murk.loaditems {
  ...
  murk.fadewind @murk.itemnotifynoto
}
