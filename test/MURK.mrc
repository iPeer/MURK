HTTP/1.1 200 OK
Date: Sat, 29 Jan 2011 13:25:42 GMT
Server: Apache
Last-Modified: Wed, 26 Jan 2011 15:46:08 GMT
ETag: "3488783-18e2a-49ac1bb43f000"
Accept-Ranges: bytes
Content-Length: 101930
Connection: close
Content-Type: application/marc

; ************************************************************************
; *  MURK has been written & tested on mIRC versions 7.15, 7.16b & 7.17  *
; ************************************************************************
menu * {
  MURK Hub: {
...
on *:start:{
  if ($player.option(AutoUpdate)) { .timerMRUKAutoUpdate 0 1800 murk.read.roll }
}
...
alias -l player.levelat {
  ...
  while (%mx <= %level) {
    var %txp $calc($floor($calc(%mx + 300 * 2 ^ (%mx / 7))) / 5) 
    inc %murk.exp %txp
    inc %mx
  }
...
alias -l player.degradeitems {
  var %type $1, %damage $floor($2), %weapon $player.weaponbase, %armour $player.armourbase, %shield $player.shieldbase
...
alias -l player.equipitem {
  if ($readini(MURK\data\Item.ini,$1,IsWeapon)) {
    ...
    if ($hget(MURK.Player,$replace($player.weaponbase,$chr(32),-)) == $null && $player.weaponbasehp) { hadd MURK.Player $replace($player.weaponbase,$chr(32),-) $player.weaponbasehp }
  }
  elseif ($readini(MURK\data\Item.ini,$1,IsArmour)) {
    var %name $readini(MURK\data\Item.ini,$1,Name)
  ...
  elseif ($readini(MURK\data\Item.ini,$1,IsShield)) {
    var %name $readini(MURK\data\Item.ini,$1,Name)
    ...
    if (!$dialog(MURK.Armoury)) { murkc Invalid Weapon, Armour or Shield ID. }
  }
}
...
alias -l murk.moveplayer {
  ...
  if (%challengeplayer) {
    ...
    if ($murk.checkformonster(%new.playerx,%new.playery)) { murk.displaywind @murk.monsterchallenge }
  }
...
alias -l player.useaid {
  if ($readini(MURK\data\Item.ini,$1,IsAid) && $player.checkquant($1,1)) {
    var %itemname $replace($readini(MURK\data\Item.ini,$1,Name),$chr(32),-)
...
alias -l player.checkquant {
  ...
  if ($readini(MURK\data\Item.ini,$1,Name)) {
    ...
    noop $regex($hget(MURK.Player,Inventory),$($+(/,$iif($numtok(%murk.currentinventory,44) > 1 && $gettok($gettok(%murk.currentinventory,1,64),1,44) != %murk.itemname,$chr(44)),%murk.itemname,@,([0-9\.]+)/)))      
...
alias -l player.completequest {
  ...
  if ($1 || $readini(MURK\data\Quest.ini,$1)) {
    var %murk.QuestId $1, %murk.QuestName $readini(MURK\data\Quest.ini,$1,Name), %murk.completequests $hget(MURK.Player,Quests)
...
alias -l player.additem {
  ...
  if ($readini(MURK\data\Item.ini,$1,Name)) {
    ...
    if ($+($iif($numtok(%murk.currentinventory,44) > 1 && $gettok($gettok(%murk.currentinventory,1,64),1,44) != %murk.itemname,$chr(44)),%murk.itemname,@) isin %murk.currentinventory) {
      noop $regex($hget(MURK.Player,Inventory),$($+(/,%murk.itemname,@,([0-9\.]+)/)))      
      var %murk.currentitem $regml(1)
...
alias murk.displaywind {
  ...
  var %murk.windname $iif($left($remove($1,-q:),6) != @murk.,@murk. $+ $remove($1,-q:),$remove($1,-q:)),$&
    %murk.winwidth $readini(MURK\data\Windows.ini,%murk.windname,width),$&
    ...
    %murk.winpos $iif($readini(MURK\data\Windows.ini,%murk.windname,WinPos),$v1,-1 -1), $&
  ...
  if ($istok(@murk.ipeerftwconnecting @murk.loaded @murk.statsupdated @murk.statsupdating @murk.saving @murk.itemnotify,%murk.windname,32) || $window(%murk.windname) && $left($1,3) != -q: && %murk.wintimeout) { inc -eu10 %murk.windowqueue | .timermurkwindowqueue. [ $+ [ %murk.windowqueue ] ] -io 1 $calc((%murk.windowqueue -1) * %murk.wintimeout) murk.displaywind $+(-q:,$1-) }
  else {
    window $+(-,$iif(%murk.winpos == -1 -1,C),$iif($istok(debug none,%murk.winstyle,32) || !$appactive || $istok(@murk.monsterchallenge @murk.founditem @murk.trader @murk.tradertimeout,%murk.windname,32),h),Badfoipw0k0) +dL %murk.windname %murk.winpos %murk.winwidth %murk.winheight
  ...
  if (%murk.wintimeout) .timerclose. [ $+ [ %murk.windname ] ] 1 %murk.wintimeout murk.fadewind %murk.windname
...
on *:SOCKREAD:MURK.POST.*: {
  ...
  if ($sockerr) {
    ...
    if (*Database Updated* iswm %sockreader || *Added to* iswm %sockreader) {
      ...
      murkc Staus Updated.
      murk.displaywind @murk.statsupdated
      sockclose $sockname
    }
...
on *:SOCKOPEN:MURKDownload.*: {
  did -a MURK.Update 6 Downloading: %murk.file. [ $+ [ $gettok($sockname,2,46) ] ]
...
on *:SOCKREAD:MURKDownload.*: {
  if ($sockerr) {
    ...
    if (HTTP* iswmcs %sockreader && *200* !iswm %sockreader) {
      did -a MURK.Update 6 %murk.file. [ $+ [ $gettok($sockname,2,46) ] ] $+($gettok(%sockreader,2,32),'d)
      did -c MURK.Update 6 $did(MURK.Update,6).lines
      did -u MURK.Update 6 $did(MURK.Update,6).lines
    ...
    if ($lof($qt($+(MURK\,%murk.savein. [ $+ [ $gettok($sockname,2,46) ] ],%file. [ $+ [ $gettok($sockname,2,46) ] ]))) >= $sock($sockname).mark) {
      ...
      did -a MURK.Update 6 Download Complete: %murk.file. [ $+ [ $gettok($sockname,2,46) ] ]
      did -c MURK.Update 6 $did(MURK.Update,6).lines
      did -u MURK.Update 6 $did(MURK.Update,6).lines
      unset %murk.*. [ $+ [ $gettok($sockname,2,46) ] ]
      sockclose $sockname
...
dialog MURK.Armoury {
  ...
  button "Leave Armoury", 7, 47 144 44 12, cancel
  box "Player Status", 8, 9 73 121 70
  text "Min/Max damage", 9, 14 81 110 8
  text "999/999", 10, 14 90 72 8
  text "Stats", 11, 14 103 109 8
  text "HP: 9999 (+9999, +9999)", 12, 14 113 65 8
  text "AP: 9999 (+9999, +9999)", 13, 14 122 65 8
  text "CC: 9999 (+9999, +9999)", 14, 14 131 65 8
...
on *:dialog:MURK.Armoury:*:*:{
  if ($devent == init) {
    var %inventory $hget(MURK.Player,Inventory), %t 1, %w 1,%a 1,%s 1
    did -a $dname 1 Hands
    ...
    while ($gettok(%inventory,%t,44)) {
      ...
      if ($readini(MURK\data\Weapon.ini,$replace(%itemname,$chr(32),-),ID)) {
        did -a $dname 1 %itemname
        inc %w
        if (%itemname == $player.weapon) { %murk.weapon = %w }
      }
      if ($readini(MURK\data\Armour.ini,$replace(%itemname,$chr(32),-),ID)) {
        did -a $dname 2 %itemname
        inc %a
        if (%itemname == $player.armour) { %murk.armour = %a }
      }
      if ($readini(MURK\data\Shield.ini,$replace(%itemname,$chr(32),-),ID)) {
    ...
    murkc %murk.weapon \ %murk.armour \ $iif(%murk.shield,$v1,1)
    ...
    var %ahp $readini(MURK\data\Armour.ini,$replace($player.armour,$chr(32),-),HPBoost), $&
      %acc $readini(MURK\data\Armour.ini,$replace($player.armour,$chr(32),-),CCBoost), $&
      %aap $readini(MURK\data\Armour.ini,$replace($player.armour,$chr(32),-),APBoost), $&
      %shp $readini(MURK\data\Shield.ini,$replace($player.shield,$chr(32),-),HPBoost), $&
      %sap $readini(MURK\data\Shield.ini,$replace($player.shield,$chr(32),-),APBoost), $&
      %scc $readini(MURK\data\Shield.ini,$replace($player.shield,$chr(32),-),CCBoost)
    did -ra $dname 12 HP: $player.hp ( $+ $+($iif($left(%ahp,1) != -,$+(+,%ahp),%ahp),$chr(44),$chr(32),$iif($left(%shp,1) != -,$+(+,%shp),%shp)) $+ )
    did -ra $dname 13 AP: $player.ap ( $+ $+($iif($left(%aap,1) != -,$+(+,%aap),%aap),$chr(44),$chr(32),$iif($left(%sap,1) != -,$+(+,%sap),%sap)) $+ )
    did -ra $dname 14 CC: $player.cc ( $+ $+($iif($left(%acc,1) != -,$+(+,%acc),%acc),$chr(44),$chr(32),$iif($left(%scc,1) != -,$+(+,%scc),%scc)) $+ )
  }
  if ($devent == sclick) {
    if ($did == 1) {
...
dialog MURK.Fight {
  ...
  text "CC:", 8, 168 12 31 8
  text "AP:", 9, 136 12 31 8
  text "LVL:", 10, 104 12 31 8
  ...
  button "Run (1337 coins)", 15, 18 52 67 12
...
on *:dialog:MURK.Fight:sclick:*:{
  ...
  if ($did == 15) { 
    if ($did(15) == Close) { dialog -x $dname }
    else { .timerMurk.Combat.Monster.Attack off | player.removeitem 1 $ceil($player.mhp) | hdel MURK.Player Monster.HP | dialog -x $dname }
  }
  if ($did == 14) { 
    if (!$dialog(MURK.Hub)) { dialog -md MURK.Hub MURK.Hub }
    else dialog -ve MURK.Hub
...
alias -l murk.startcombat {
  ...
  %monster = $iif($1 && $1 !== BOSS,$1,$ini(MURK\data\Monster.ini,$r(1,$ini(MURK\data\Monster.ini,0))))
  var %d MURK.Fight, %monsterulvl $calc($player.level * $readini(MURK\data\Monster.ini,%monster,LPPL)), %monsterlvl $floor($calc(%monsterulvl $iif($r(1,2) == 1,+,-) (%monsterulvl *.10)))
  var %monsterhp $calc(%monsterlvl * $readini(MURK\data\Monster.ini,%monster,HPPL)), $&
    %monsterap $calc(%monsterlvl * $readini(MURK\data\Monster.ini,%monster,LPPL)), %monstercc $calc(%monsterlvl * $readini(MURK\data\Monster.ini,%monster,CCPL)) 
  if (%monsterulvl == 0 && $player.level > 0) { murk.displaywind @murk.criticalerror Monster $qt(%monster) does not exist. | halt }
  dialog -md %d %d
  if (!$player.chp) hadd -m MURK.Player CHP $player.hp
  did -ra %d 2 HP: $player.chp ( $+ $player.hp $+ )
  did -ra %d 5 LVL: $player.level
  did -ra %d 6 AP: $player.ap
  did -ra %d 7 CC: $player.cc
...
alias -l murk.combat.player {
  var %monsterattacktime $iif($player.option(MonsterDelay),$v1,3), %d MURK.Fight, $&
    ...
    %playerweapon $player.weapon, %playermind $readini(MURK\data\Weapon.ini,$replace(%playerweapon,$chr(32),-),MinDam), $&
    %playermaxd $readini(MURK\data\Weapon.ini,$replace(%playerweapon,$chr(32),-),MaxDam), $&
    %monsterlvl $gettok($did(MURK.Fight,10),2,32), $&
    %monsterbasehp $calc(%monsterlvl * $readini(MURK\data\Monster.ini,%monster,HPPL)), $&
    %monsterlootchance $readini(MURK\data\Monster.ini,%monster,LootChance), $&
    %monsterloottotal $numtok($readini(MURK\data\Monster.ini,%monster,Loot),44), $&
    %monsterloot $readini(MURK\data\Monster.ini,%monster,Loot), $&
    %monsterrarechance $readini(MURK\data\Monster.ini,%monster,RareChance), $&
    %monsterrareloottotal $numtok($readini(MURK\data\Monster.ini,%monster,RareLoot),44) $&
    %monsterrareloot $readini(MURK\data\Monster.ini,%monster,RareLoot)
  set %murk.iscritical $iif($r(1,1000) <= $iif(%playercc > 900,900,$v1),1,0)
  if ($player.weaponhp == 0 && %playerdegrades) { var %playermaxd $calc(%playermaxd * .10), %playermind $calc(%playermind * .10) }
  ...
  if ($gettok($player.shield,1-2,32) == Champion's Shield && $r(1,8) == 8) { 
    if ($player.weapon != Upgraded Champion's Shield) { 
  ...
  if (Poisoned* iswm $player.weapon) { 
    if (%murk.monsterpoisoned) {
      inc %murk.playerhit $calc(%monsterhp * .10) 
  ...
  ; - Apply the monster's AP bonus, providing the iuser isnt using  the "Thrio Sword" -
  ...
  if ($player.mhp <= 0) {
    ...
    did -ve %d 15
    if ($dialog(MURK.Hub)) { did -ra MURK.Hub 30,29,28,27 Coins $bytes($player.checkquant(1,return),b) }
    ...
    if ($dialog(MURK.Hub)) { murk.populatehub }
    did -b %d 13,14
    murkc Coins to add: $floor($calc(%monsterbasehp * ( $player.level * .10)))
    player.additem 1 $floor($calc(%monsterbasehp * ( $player.level * .10)))
    player.addexp %monsterbasehp
...
alias -l murk.combat.monster {
  var %d MURK.Fight, %monsterulvl $calc($player.level * $readini(MURK\data\Monster.ini,%monster,LPPL)), $&
    %monsterlvl $gettok($did(MURK.Fight,10),2,32), $&
    %monstercc $calc(%monsterlvl * $readini(MURK\data\Monster.ini,%monster,CCPL)), %monstermax $calc(%monsterlvl * $readini(MURK\data\Monster.ini,%monster,MDPL))
...
dialog MURK.Hub {
  ...
  button "Report Bug", 36, 283 209 113 25, tab 1
  button "Options", 39, 87 239 75 25, tab 1
  button "Troubleshooting FAQ", 37, 283 183 113 25, tab 1
  text "Next trader in: -:-:-", 38, 12 103 384 16, tab 1
  tab "Weapons", 3
  list 6, 12 40 184 220, tab 3 size vsbar
  button "Open Armoury", 8, 208 216 160 24, tab 3
  text "This window simply lets you view which items in this category you're carrying. If you wish to customise further please use the Armoury.", 11, 204 44 164 72, tab 3
  ...
  text "This window simply lets you view which items in this category you're carrying. If you wish to customise further please use the Armoury.", 12, 204 44 164 72, tab 4
  ...
  text "Coins: 99999", 29, 208 168 160 16, tab 5 center
  text "", 32, 194 98 186 56, tab 5 center
  tab "Misc. Items", 10
  ...
  text "Build: 0", 13, 311 254 89 16, disable right
  text "Loading MOTD...", 41, 1 275 404 31, center
  text "00:00", 42, 12 82 384 16, hide center
}
...
on *:dialog:MURK.Hub:sclick:*:{
  ...
  if ($did == 14) { 
    var %ID $readini(MURK\data\Aid.ini,$replace($left($gettok($did(14).seltext,1,40),-1),$chr(32),-),ID), %val $readini(MURK\data\Item.ini,%ID,VAL), %Desc $readini(MURK\data\Item.ini,%ID,DES)
  ...
  if ($did == 21) {
    var %ID $readini(MURK\data\Aid.ini,$replace($left($gettok($did(14).seltext,1,40),-1),$chr(32),-),ID), %val $readini(MURK\data\Item.ini,%ID,VAL)
    player.additem %ID 1
  ...
  if ($did == 6) {
    var %ID $murk.getitemidbyname($left($gettok($did(6).seltext,1,40),-1)), %val $readini(MURK\data\Item.ini,%ID,VAL)
    did -ra $dname 16 Sell Selected ( $+ %val $+ )
    if ($player.checkquant(%ID,1)) did -e $dname 16
    else did -b $dname 16
  }
  if ($did == 16) {
    var %name $left($gettok($did(6).seltext,1,40),-1), %ID $murk.getitemidbyname(%name), %val $readini(MURK\data\Item.ini,%ID,VAL)
    player.additem 1 %val
    player.removeitem %ID 1
    var %line $did(6).sel
  ...
  if ($did == 7) {
    var %ID $murk.getitemidbyname($left($gettok($did(7).seltext,1,40),-1)), %val $readini(MURK\data\Item.ini,%ID,VAL)
    did -ra $dname 17 Sell Selected ( $+ %val $+ )
    if ($player.checkquant(%ID,1)) did -e $dname 17
    else did -b $dname 17
  }
  if ($did == 17) {
    var %name $left($gettok($did(7).seltext,1,40),-1), %ID $murk.getitemidbyname(%name), %val $readini(MURK\data\Item.ini,%ID,VAL)
    player.additem 1 %val
    player.removeitem %ID 1
    var %line $did(7).sel
  ...
  if ($did == 15) {
    var %ID $murk.getitemidbyname($left($gettok($did(15).seltext,1,40),-1)), %val $readini(MURK\data\Item.ini,%ID,VAL), %desc $iif($readini(MURK\data\Item.ini,%ID,DES),$v1,No description.)
  ...
  if ($did == 19) {
    var %ID $murk.getitemidbyname($left($gettok($did(15).seltext,1,40),-1)), %val $readini(MURK\data\Item.ini,%ID,VAL)
    player.additem 1 %val
    player.removeitem %ID 1
    var %line $did(15).sel
...
alias murk.populatehub {
  ...
  if (!$murk.player) { did -ra %d 6,7,14,15 You must load or start | did -a %d 6,7,14,15 a game before items will | did -a %d 6,7,14,15 be listed here. }
    ...
    while ($gettok(%inventory,%tok,44)) {
      %token = $gettok(%inventory,%tok,44)
      if (Coins* !iswm %token) {
        if ($readini(MURK\data\Weapon.ini,$replace($gettok(%token,1,64),$chr(32),-),ID)) { did -a %d 6 $gettok(%token,1,64) $iif($gettok(%token,2,64) > 0,$+($chr(40),$gettok(%token,2,64),$chr(41))) }
        elseif ($readini(MURK\data\Armour.ini,$replace($gettok(%token,1,64),$chr(32),-),ID) || $readini(MURK\data\Shield.ini,$replace($gettok(%token,1,64),$chr(32),-),ID)) { did -a %d 7 $gettok(%token,1,64) $iif($gettok(%token,2,64) > 0,$+($chr(40),$gettok(%token,2,64),$chr(41))) }
        elseif ($readini(MURK\data\Aid.ini,$replace($gettok(%token,1,64),$chr(32),-),ID)) { did -a %d 14 $gettok(%token,1,64) $iif($gettok(%token,2,64) > 0,$+($chr(40),$gettok(%token,2,64),$chr(41))) }
...
alias -l murk.populatemods {
  ...
  while ($gettok(%inventory,%tok,44)) {
    %token = $gettok(%inventory,%tok,44)
    if ($readini(MURK\data\Weapon.ini,$replace($gettok(%token,1,64),$chr(32),-),ID) $&
      || $readini(MURK\data\Armour.ini,$replace($gettok(%token,1,64),$chr(32),-),ID) $&
      || $readini(MURK\data\Shield.ini,$replace($gettok(%token,1,64),$chr(32),-),ID)) { 
      did -a %d 1 $gettok(%token,1,64) $iif($gettok(%token,2,64) > 0,$+($chr(40),$gettok(%token,2,64),$chr(41))) 
    }
    elseif (Coins* !iswm %token || !$readini(MURK\data\Aid.ini,$replace($gettok(%token,1,64),$chr(32),-),ID)) { did -a %d 2 $gettok(%token,1,64) $iif($gettok(%token,2,64) > 0,$+($chr(40),$gettok(%token,2,64),$chr(41))) }
    inc %tok
  }
  unset %token
}
...
alias -l murk.displayvendor {
  if ($murk.player) {
    var %next $r(1800,3600), %next2 $calc($ctime + %next)
...
alias -l murk.hubupdatevendor {
  if ($murk.player) {
    ...
    if ($dialog(MURK.Hub)) { 
      did -ra MURK.Hub 38 Next vendor in: $duration($calc($hget(MURK.Player,NextVendor) - $ctime),3)
...
on *:dialog:MURK.Trade:scroll:6: {
  if ($did(1).seltext) {
    ...
    did -ra $dname 4 Steal Selected $+($chr(40),$calc(36 - $did(6).sel),$chr(37),$chr(41))
...
on *:dialog:MURK.Trade:sclick:*:{
  ...
  if ($did == 3) { 
    ...
    did -c $dname 1 %line
    if ($gettok($did(1).seltext,2,40) == $($+(0,$chr(41)))) { did -d $dname 1 %line | did -b $dname 3 }
  }
  if ($did == 4) { 
